...
describe('BootstrapPanel (sidebar variant)', () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks()
    mockUseTriggerBootstrap.mockReturnValue({ mutate: mutateFn, mutateAsync: mutateAsyncFn, isPending: false })
  ...
  it('renders idle action row when no job exists', () => {
    ...
    expect(screen.getByText('正在准备全书内容')).toBeTruthy()
  })
  ...
  it('treats initialized as the sole source of truth for primary actions', async () => {
    mutateAsyncFn.mockResolvedValue({ ...baseJob, status: 'pending' as const })
    ...
    await userEvent.click(screen.getByText(/执行失败/))

    expect(mutateAsyncFn).toHaveBeenCalledWith({ mode: 'initial' })
  })
  ...
  it('does not mark zero-result extraction as review-ready', () => {
    const lifecycleSpy = vi.fn()
    mockUseBootstrapStatus.mockReturnValueOnce({
      data: {
        ...baseJob,
    ...
    const view = render(
      <QueryClientProvider client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}>
        <UiLocaleProvider>
          <ToastProvider>
            <BootstrapPanel novelId={1} onLifecycleChange={lifecycleSpy} />
          </ToastProvider>
        </UiLocaleProvider>
      </QueryClientProvider>,
    )
  ...
  it('shows English LLM error copy when bootstrap trigger fails in the en locale', async () => {
    localStorage.setItem('novwr_ui_locale', 'en')
    document.documentElement.lang = 'en'
    mutateAsyncFn.mockRejectedValue(
  ...
  it('calls trigger with initial payload on idle row click', async () => {
    ...
    expect(trackHostedAnalyticsEventMock).toHaveBeenCalledWith('bootstrap_trigger', expect.objectContaining({
      novelId: 1,
  ...
  it('disables action row when trigger is pending', () => {
    mockUseBootstrapStatus.mockReturnValue({ data: null, isLoading: false })
    mockUseTriggerBootstrap.mockReturnValue({ mutate: mutateFn, mutateAsync: mutateAsyncFn, isPending: true })
    renderPanel()
  ...
  it('fires pending callbacks immediately before the bootstrap request resolves', async () => {
    mockUseBootstrapStatus.mockReturnValue({ data: null, isLoading: false })
    let resolveMutation: ((value: BootstrapJobResponse) => void) | null = null
    mutateAsyncFn.mockImplementation(() => new Promise<BootstrapJobResponse>((resolve) => {
      resolveMutation = resolve
    }))

    const onTriggerStart = vi.fn()
    ...
    expect(onTriggerStart).toHaveBeenCalledTimes(1)
    expect(onJobAccepted).not.toHaveBeenCalled()

    resolveMutation?.({ ...baseJob, status: 'pending' })
