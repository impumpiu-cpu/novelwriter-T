...
Настройки читаются из переменных окружения / файла ``.env`` (pydantic-settings).
Здесь же: ограничение числа глав контекста для продолжения, разбор списка
инвайт-кодов hosted-режима и вычисляемые свойства (``is_production`` и т. п.).
"""
