"""Фоновый worker очереди задач.

Отдельный процесс (второй контейнер в docker-compose), в цикле опрашивающий
очередь задач в БД. Приоритет за один проход: импорт романов → отложенные
сборки индекса всей книги → пересборки индекса → задачи bootstrap
(извлечение модели мира из глав). Механизм аренды (lease) с heartbeat
защищает от двойного исполнения и зависших задач.
"""
