...
"""Жизненный цикл индекса всей книги (window index).

Управляет постановкой, арендой и выполнением задач сборки/пересборки индекса:
книга разбивается на перекрывающиеся окна текста, по которым Copilot ищет
свидетельства. Контракт ревизий гарантирует, что индекс помечается устаревшим
при изменении глав или языка романа.
"""
