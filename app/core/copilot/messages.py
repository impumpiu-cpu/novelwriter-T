...
# Полный русский каталог рантайм-сообщений Copilot (отображаются в интерфейсе).
_RU: dict[CopilotTextKey, str] = {
    CopilotTextKey.RUN_FAILED: "Запуск Copilot не удался. Попробуйте позже.",
    CopilotTextKey.RUN_INTERRUPTED: "Запуск Copilot потерял активную фоновую аренду. Попробуйте позже.",
    CopilotTextKey.RUN_RESEARCHING: "Идёт исследование. Ожидаем, пока модель решит, нужны ли инструменты...",

    CopilotTextKey.TEXT_DESCRIPTION_LABEL: "Описание",
    CopilotTextKey.TEXT_ATTRIBUTES_LABEL: "Атрибуты",
    CopilotTextKey.TEXT_CONSTRAINTS_LABEL: "Ограничения",
    CopilotTextKey.TEXT_NO_DESCRIPTION: "(без описания)",
    CopilotTextKey.TEXT_RESOURCE_ENTITY: "сущность",
    CopilotTextKey.TEXT_RESOURCE_RELATIONSHIP: "связь",
    CopilotTextKey.TEXT_RESOURCE_SYSTEM: "система",
    CopilotTextKey.TEXT_NEW_RESOURCE: "Новая запись: {resource_label}",
    CopilotTextKey.TEXT_FIELD_NAME: "Название",
    CopilotTextKey.TEXT_FIELD_ENTITY_TYPE: "Тип",
    CopilotTextKey.TEXT_FIELD_DESCRIPTION: "Описание",
    CopilotTextKey.TEXT_FIELD_ALIASES: "Псевдонимы",
    CopilotTextKey.TEXT_FIELD_RELATIONSHIP_LABEL: "Метка связи",
    CopilotTextKey.TEXT_FIELD_VISIBILITY: "Видимость",
    CopilotTextKey.TEXT_FIELD_CONSTRAINTS: "Ограничения",
    CopilotTextKey.TEXT_FIELD_DISPLAY_TYPE: "Тип отображения",
    CopilotTextKey.TEXT_ATTRIBUTE_FIELD_LABEL: "Атрибут · {key}",

    CopilotTextKey.TRACE_EMPTY_QUERY: "(пустой запрос)",
    CopilotTextKey.TRACE_RETRIEVAL_STEP_INCOMPLETE: "Шаг поиска не завершился: {error}",
    CopilotTextKey.TRACE_FIND: "Поиск «{query}»",
    CopilotTextKey.TRACE_FIND_SCOPE_SUFFIX: " (область: {scope})",
    CopilotTextKey.TRACE_FIND_TOTAL_SUFFIX: ", найдено групп подсказок: {count}",
    CopilotTextKey.TRACE_OPEN: "Развёрнут дополнительный контекст",
    CopilotTextKey.TRACE_OPEN_SOURCE_SUFFIX: ", добавлено источников: {count}",
    CopilotTextKey.TRACE_READ_TARGETS: "Чтение целей сеттинга: {count}",
    CopilotTextKey.TRACE_READ_RESULTS_SUFFIX: ", возвращено результатов: {count}",
    CopilotTextKey.TRACE_REFRESH_SNAPSHOT: "Обновление снимка текущего сеттинга",
    CopilotTextKey.TRACE_REFRESH_ENTITIES: "сущностей {count}",
    CopilotTextKey.TRACE_REFRESH_RELATIONSHIPS: "связей {count}",
    CopilotTextKey.TRACE_REFRESH_DRAFTS: "черновиков {count}",
    CopilotTextKey.TRACE_REFRESH_COUNTS_SUFFIX: ", {counts}",
    CopilotTextKey.TRACE_REFRESH_CONTEXT_REFRESHED_SUFFIX: ": контекст обновлён",
    CopilotTextKey.TRACE_GENERIC_TOOL_COMPLETED: "Шаг поиска «{tool_name}» выполнен",

    CopilotTextKey.TRACE_TOOL_MODE_USED_STEPS: "В этом запуске информация собрана пошаговым поиском, шагов: {count}",
    CopilotTextKey.TRACE_TOOL_MODE_DIRECT: "Дополнительные шаги поиска не понадобились; модель завершила анализ напрямую",
    CopilotTextKey.TRACE_TOOL_COMPLETED_FALLBACK: "Инструмент {tool_name}: выполнен",
    CopilotTextKey.TRACE_ANALYZE_RUNNING: "Систематизируем результаты поиска и формируем ответ...",
    CopilotTextKey.TRACE_TOOL_LOOP_COMPLETED: "Инструментальное исследование этого запуска завершено",
    CopilotTextKey.TRACE_ONE_SHOT_UNSUPPORTED: "Текущая модель не поддерживает пошаговый поиск, выполнен прямой анализ",
    CopilotTextKey.TRACE_ONE_SHOT_FAILED: "Сбой пошагового поиска ({reason}), выполнен прямой анализ",
    CopilotTextKey.TRACE_EVIDENCE_PREPARED: "Подготовлено оснований для показа: {count}",
    CopilotTextKey.TRACE_ANALYSIS_COMPLETED: "Анализ завершён, предложений сформировано: {count}",

    CopilotTextKey.SCOPE_CHAPTER_WINDOW_TITLE: "Глава {chapter_number} · позиция {start}-{end}",
    CopilotTextKey.SCOPE_CHAPTER_TAIL_TITLE: "Глава {chapter_number} · конец",
    CopilotTextKey.SCOPE_CHAPTER_MENTIONS_ENTITY: "Содержит упоминание «{entity_name}»",
    CopilotTextKey.SCOPE_RECENT_CHAPTER_CONTEXT: "Контекст последних глав",
    CopilotTextKey.SCOPE_STALE_RECENT_CHAPTER_CONTEXT: "Главы обновились, временно используется контекст последних глав",
    CopilotTextKey.SCOPE_MISSING_RECENT_CHAPTER_CONTEXT: "Содержимое всей книги ещё готовится, временно используется контекст последних глав",
    CopilotTextKey.SCOPE_FAILED_RECENT_CHAPTER_CONTEXT: "Не удалось систематизировать содержимое всей книги, временно используется контекст последних глав",
    CopilotTextKey.SCOPE_ENTITY_TITLE: "Сущность · {entity_name}",
    CopilotTextKey.SCOPE_ENTITY_TARGET_REASON: "Текущая целевая сущность исследования",
    CopilotTextKey.SCOPE_RELATIONSHIP_EXCERPT: "Связь: {source_name} → {label} → {target_name}. {description}",
    CopilotTextKey.SCOPE_RELATIONSHIP_TARGET_REASON: "Известные связи, относящиеся к целевой сущности",
    CopilotTextKey.SCOPE_DRAFT_ENTITY_EXCERPT: "[Черновик сущности] {entity_name} ({entity_type})",
    CopilotTextKey.SCOPE_DRAFT_ENTITY_TITLE: "Черновик сущности · {entity_name}",
    CopilotTextKey.SCOPE_DRAFT_ENTITY_REASON: "Сущность из текущего набора черновиков",
    CopilotTextKey.SCOPE_DRAFT_RELATIONSHIP_EXCERPT: "[Черновик связи] {source_name} --[{label}]--> {target_name}",
    CopilotTextKey.SCOPE_DRAFT_RELATIONSHIP_TITLE: "Черновик связи · {label}",
    CopilotTextKey.SCOPE_DRAFT_RELATIONSHIP_REASON: "Связь из текущего набора черновиков",
    CopilotTextKey.SCOPE_DRAFT_SYSTEM_EXCERPT: "[Черновик системы] {system_name}",
    CopilotTextKey.SCOPE_DRAFT_SYSTEM_TITLE: "Черновик системы · {system_name}",
    CopilotTextKey.SCOPE_DRAFT_SYSTEM_REASON: "Система из текущего набора черновиков",

    CopilotTextKey.TOOL_UNKNOWN_TOOL: "Неизвестный инструмент: {tool_name}",
    CopilotTextKey.TOOL_UNKNOWN_PACK: "Неизвестный pack_id: {pack_id}. Сначала вызовите find().",
    CopilotTextKey.TOOL_OPEN_MANY_NO_PACKS: "open_many требует хотя бы один корректный pack_id.",
    CopilotTextKey.TOOL_OPEN_MANY_TOO_MANY_PACKS: "open_many принимает не более {max_count} разных pack_id за вызов; сократите список и повторите.",
    CopilotTextKey.TOOL_OPEN_MANY_FAILED_COUNT: "open_many выполнен не полностью: не удалось открыть {failed_count}/{requested_count} pack_id. Проверьте results.",
    CopilotTextKey.TOOL_ISSUE_MISSING_DESCRIPTION: "Пустое описание",
    CopilotTextKey.TOOL_ISSUE_NO_ALIASES: "Нет псевдонимов",
    CopilotTextKey.TOOL_ISSUE_NO_ATTRIBUTES: "Нет атрибутов",
    CopilotTextKey.TOOL_ISSUE_NO_CONSTRAINTS: "Нет ограничений",
    CopilotTextKey.TOOL_DRAFT_ENTITY_ISSUES_EXCERPT: "[Черновик сущности] {entity_name} ({entity_type}) — проблемы: {issues}",
    CopilotTextKey.TOOL_DRAFT_RELATIONSHIP_MISSING_DESCRIPTION_EXCERPT: "[Черновик связи] {source_name} --[{label}]--> {target_name} — пустое описание",
    CopilotTextKey.TOOL_DRAFT_SYSTEM_ISSUES_EXCERPT: "[Черновик системы] {system_name} — проблемы: {issues}",

    CopilotTextKey.SUGGESTION_SYNTH_ENTITY_TITLE: "Добавить связанную сущность «{entity_name}»",
    CopilotTextKey.SUGGESTION_SYNTH_ENTITY_SUMMARY: "Добавить недостающую сущность «{entity_name}», чтобы предложение о связи можно было применить.",
    CopilotTextKey.SUGGESTION_FALLBACK_TITLE: "Предложение {index}",
    CopilotTextKey.SUGGESTION_REASON_STALE: "Содержимое этого предложения только что изменилось. Обновите страницу и попробуйте снова.",
    CopilotTextKey.SUGGESTION_REASON_DRAFT_ONLY: "Этот шаг может напрямую приводить в порядок только черновики; подтверждённое содержимое редактируйте на его странице.",
    CopilotTextKey.SUGGESTION_REASON_CANNOT_APPLY_DIRECT: "Это предложение пока нельзя принять напрямую. Продолжите другим способом.",
    CopilotTextKey.SUGGESTION_REASON_DRAFT_CREATE_DISALLOWED: "Это рабочее пространство предназначено для приведения в порядок существующих черновиков; для создания новых записей вернитесь к обычному редактированию.",
    CopilotTextKey.SUGGESTION_REASON_NOT_DIRECTLY_APPLICABLE: "Это предложение пока нельзя принять напрямую.",
    CopilotTextKey.SUGGESTION_CREATE_REASON_ENTITY_INCOMPLETE: "Это предложение о сущности неполное, его пока нельзя принять.",
    CopilotTextKey.SUGGESTION_CREATE_REASON_ENTITY_NAME_COLLISION: "Это имя слишком похоже на существующее. Измените его перед подтверждением.",
    CopilotTextKey.SUGGESTION_CREATE_REASON_RELATIONSHIP_INCOMPLETE: "Информация об этой связи неполная, её пока нельзя принять.",
    CopilotTextKey.SUGGESTION_CREATE_REASON_RELATIONSHIP_DEPENDENCY: "Эта связь зависит от неподтверждённых сущностей или настроек. Сначала подтвердите связанные сущности, затем — саму связь.",
    CopilotTextKey.SUGGESTION_CREATE_REASON_RELATIONSHIP_CONFLICT: "Эта связь дублирует существующие данные или конфликтует с ними, её пока нельзя принять.",
    CopilotTextKey.SUGGESTION_CREATE_REASON_SYSTEM_INCOMPLETE: "Это предложение о системе неполное, его пока нельзя принять.",
    CopilotTextKey.SUGGESTION_CREATE_REASON_SYSTEM_NAME_COLLISION: "Название этой системы слишком похоже на существующее. Измените его перед подтверждением.",

    CopilotTextKey.APPLY_ERROR_SUGGESTION_NOT_FOUND: "Это предложение уже недействительно. Сгенерируйте его заново.",
    CopilotTextKey.APPLY_ERROR_ALREADY_APPLIED: "Это предложение уже было подтверждено.",
    CopilotTextKey.APPLY_ERROR_STALE: "Содержимое только что изменилось. Обновите страницу и попробуйте снова.",
    CopilotTextKey.APPLY_ERROR_DEPENDENCY_FAILED: "Эта связь зависит от неподтверждённых сущностей или настроек. Сначала подтвердите связанные сущности, затем — саму связь.",
    CopilotTextKey.APPLY_ERROR_GENERIC: "Подтверждение не удалось. Попробуйте позже.",

    CopilotTextKey.WORKSPACE_EVIDENCE_COMPILED: "Собрано из связанных подсказок",
    CopilotTextKey.WORKSPACE_EVIDENCE_COMPILED_MULTIPLE: "Собрано из {count} связанных подсказок",
}
...
register_copilot_messages("ru", _RU)
